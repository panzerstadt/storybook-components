import React from "react";

import styles from "./Hogwarts.module.css";

const HogwartsTicket = () => {
  return (
    <div className={styles.body}>
      <link
        href="https://fonts.googleapis.com/css?family=Jura:400,700|Questrial|Inconsolata:400,700|Montserrat:900|Old+Standard+TT:700"
        rel="stylesheet"
      />
      <div className={styles.mainContent}>
        <div className={styles.ticket}>
          <div className={styles.ticket__main}>
            <div className={styles.header}>Hogwarts Express</div>
            <div className={styles.info + " " + styles.passenger}>
              <div className={styles.info__item}>Passenger</div>
              <div className={styles.info__detail}>Hermione J. Granger</div>
            </div>
            <div className={styles.info + " " + styles.platform}>
              {" "}
              <span>Depart </span>
              <span>from </span>
              <span>platform</span>
              <div className={styles.number}>
                <div>9</div>
                <div>
                  {" "}
                  <span>3</span>
                  <span>4</span>
                </div>
              </div>
            </div>
            <div className={styles.info + " " + styles.departure}>
              <div className={styles.info__item}>Depart</div>
              <div className={styles.info__detail}>King's Cross</div>
            </div>
            <div className={styles.info + " " + styles.arrival}>
              <div className={styles.info__item}>Arrive</div>
              <div className={styles.info__detail}>Hogsmeade</div>
            </div>
            <div className={styles.info + " " + styles.date}>
              <div className={styles.info__item}>Date</div>
              <div className={styles.info__detail}>1 Sep 2018</div>
            </div>
            <div className={styles.info + " " + styles.time}>
              <div className={styles.info__item}>Time</div>
              <div className={styles.info__detail}>11:00AM</div>
            </div>
            <div className={styles.info + " " + styles.carriage}>
              <div className={styles.info__item}>car</div>
              <div className={styles.info__detail}>4</div>
            </div>
            <div className={styles.info + " " + styles.seat}>
              <div className={styles.info__item}>Seat</div>
              <div className={styles.info__detail}>6B</div>
            </div>
            <div className={styles.fineprint}>
              <p>
                Boarding begins 30 minutes before departure. Snacks available
                for purchase from The Honeydukes Express.
              </p>
              <p>
                This ticket is Non-refundable • Hogwarts Express Railway
                Authority
              </p>
            </div>
            <div className={styles.snack}>
              <svg viewBox="0 -11 414.00053 414">
                <path d="m202.480469 352.128906c0-21.796875-17.671875-39.46875-39.46875-39.46875-21.800781 0-39.472657 17.667969-39.472657 39.46875 0 21.800782 17.671876 39.472656 39.472657 39.472656 21.785156-.023437 39.445312-17.683593 39.46875-39.472656zm0 0" />
                <path d="m348.445312 348.242188c2.148438 21.691406-13.695312 41.019531-35.390624 43.167968-21.691407 2.148438-41.015626-13.699218-43.164063-35.390625-2.148437-21.691406 13.695313-41.019531 35.386719-43.167969 21.691406-2.148437 41.019531 13.699219 43.167968 35.390626zm0 0" />
                <path d="m412.699219 63.554688c-1.3125-1.84375-3.433594-2.941407-5.699219-2.941407h-311.386719l-3.914062-24.742187c-3.191407-20.703125-21.050781-35.9531252-42-35.871094h-42.699219c-3.867188 0-7 3.132812-7 7s3.132812 7 7 7h42.699219c14.050781-.054688 26.03125 10.175781 28.171875 24.0625l33.800781 213.515625c3.191406 20.703125 21.050781 35.957031 42 35.871094h208.929687c3.863282 0 7-3.132813 7-7 0-3.863281-3.136718-7-7-7h-208.929687c-14.050781.054687-26.03125-10.175781-28.171875-24.0625l-5.746094-36.300781h213.980469c18.117187-.007813 34.242187-11.484376 40.179687-28.597657l39.699219-114.578125c.742188-2.140625.402344-4.511718-.914062-6.355468zm0 0" />
              </svg>
            </div>
            <div className={styles.barcode}>
              <div className={styles.barcode__scan} />
              <div className={styles.barcode__id}>001256733</div>
            </div>
          </div>
          <div className={styles.ticket__side}>
            <div className={styles.logo}>
              <p>Hogwarts Express</p>
            </div>
            <div className={styles.info + " " + styles.sideArrive}>
              <div className={styles.info__item}>Arrive</div>
              <div className={styles.info__detail}>Hogsmeade</div>
            </div>
            <div className={styles.info + " " + styles.sideDepart}>
              <div className={styles.info__item}>Depart</div>
              <div className={styles.info__detail}>King's Cross</div>
            </div>
            <div className={styles.info + " " + styles.sideDate}>
              <div className={styles.info__item}>Date</div>
              <div className={styles.info__detail}>1 Sep 2018</div>
            </div>
            <div className={styles.info + " " + styles.sideTime}>
              <div className={styles.info__item}>Time</div>
              <div className={styles.info__detail}>11:00AM</div>
            </div>
            <div className={styles.barcode}>
              <div className={styles.barcode__scan} />
              <div className={styles.barcode__id}>001256733</div>
            </div>
          </div>
        </div>
      </div>

      <aside className={styles.context}>
        <div className={styles.explanation}>
          Part of the{" "}
          <a href="https://codepen.io/collection/DQvYpQ/" target="_blank">
            CSS Grid collection here
          </a>
          .
        </div>
      </aside>
      <footer>
        <a href="https://twitter.com/meowlivia_" target="_blank">
          <i className="icon-social-twitter icons" />
        </a>
        <a href="https://github.com/oliviale" target="_blank">
          <i className="icon-social-github icons" />
        </a>
        <a href="https://dribbble.com/oliviale" target="_blank">
          <i className="icon-social-dribbble icons" />
        </a>
      </footer>
    </div>
  );
};

export default HogwartsTicket;
